...
/* 
 - so if we push one array into another then 
 that whole array gets combined as a single item in that first array 
 i.e it took that array as a single data
 - so do not use this for two array combinition
*/


// using concat()
const newConcat = arr1.concat(arr2);
console.log(newConcat);// out- [ 'a', 'b', 'c','d', 'A', 'B','C', 'D'] 
/*
  i.e it combines two arrays and returns new array...
  i.e to get answer you will have to assign new variable.
...
// as you put "..." then it no more remains array, its each elements now has become individual
...
/* 
  ...
  - but you can't just put the whole object i.e
*/
console.log(Array.from({name : "nikki"}));// out- []
/* 
  it will give an empty array.. 
  - so when you want to convert obj to arr 
    at that time you should specify if you want either "keys" or "values" to be converted into the array.
*/
...
/* 
  ...
  - way to convert diff variables into array
*/
const num1 = 100;
const num2 = 200;
const num3 = 300;
